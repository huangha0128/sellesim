...
export const WRITE_ENDPOINTS: EndpointSpec[] = [
  ...
  },
  {
    ...
    path: '/wallet/topups',
    title: '充值下单',
    desc: '发起余额充值，返回支付宝 H5 支付链接（payUrl）。支付成功后：先抵扣欠款，剩余进入余额。',
    auth: 'write',
    body: [
      { name: 'amount', type: 'number', required: true, desc: '充值金额（元，最多两位小数）' },
      { name: 'returnUrl', type: 'string', required: false, desc: '支付完成后回调跳转地址（前端页面）' },
    ],
    response: [
      { name: 'rechargeNo', type: 'string', required: true, desc: '充值单号（RC 开头）' },
      { name: 'amount', type: 'number', required: true, desc: '充值金额' },
      { name: 'payUrl', type: 'string', required: true, desc: '支付宝 H5 支付链接' },
    ],
    requestExample: JSON.stringify({ amount: 100, returnUrl: 'https://example.com/wallet' }, null, 2),
    responseExample: JSON.stringify({
      rechargeNo: 'RC20260920110001', amount: 100, payUrl: 'https://openapi.alipay.com/gateway.do?...',
    }, null, 2),
    defaultBody: { amount: 100, returnUrl: 'https://example.com/wallet' },
  },
  {
