// 简体中文
export default {
  common: {
    loading: '加载中',
    networkError: '网络错误，请重试',
    opFailed: '操作失败',
    view: '查看',
    unknown: '未知',
    know: '知道了',
    needLogin: '请先登录',
    cancel: '取消',
    dayUnit: '{d}天',
    priceFrom: '起'
  },
  tab: {
    home: '首页',
    esims: '我的 eSIM',
    profile: '我的'
  },
  pageTitle: {
    countries: '选择目的地',
    packages: 'eSIM 套餐',
    detail: '套餐详情',
    checkout: '确认订单',
    payment: '收银台',
    orders: '我的订单',
    orderDetail: '订单详情',
    esims: '我的 eSIM',
    profile: '我的',
    guide: 'eSIM 安装指南',
    email: '我的邮箱地址',
    faq: '常见问题',
    esimDetail: 'eSIM 详情',
    esimCard: 'eSIM 卡详情',
    about: '关于我们',
    supportedModels: '支持 eSIM 机型汇总',
    usageNotice: 'eSIM 使用须知',
    contact: '联系我们'
  },
  index: {
    slogan: '全球 200+ 地区流量',
    titleMain: '全球流量',
    titleSub: '一卡搞定',
    searchPlaceholder: '搜索您想去的目的地',
    searchBtn: '搜索',
    heroBadge: 'TRAVEL eSIM',
    heroTitle: '全球流量',
    heroSub: '一卡搞定',
    heroFeature1: '即时激活',
    heroFeature2: '扫码即装',
    heroFeature3: '190+ 目的地',
    plan: '流量套餐',
    instant: '即时激活',
    noRealName: '扫码即装',
    soldCount: '已售 {n}',
    hotCountries: '热门目的地',
    allCountries: '全部国家 ›',
    hotPackages: '热销套餐',
    viewMore: '查看更多 ›',
    gbTraffic: '{gb}GB 流量',
    daysValid: '{days}天有效',
    priceFrom: '¥{price} 起',
    guideTitle: '第一次用 eSIM？',
    guideSub: '3 分钟学会安装 · 全球上网不迷路',
    guideBtn: '查看指南'
  },
  profile: {
    traveler: '全球旅行者',
    clickLogin: '点击登录',
    loginBenefits: '登录后享受更多服务',
    welcomeBack: '欢迎回来',
    statEsims: '我的 eSIM',
    statOrders: '全部订单',
    statRegions: '覆盖地区',
    statusPending: '待付款',
    statusActivate: '待激活',
    statusDone: '已完成',
    statusRefunded: '已退款',
    myPurchases: '我的购买',
    allOrders: '全部订单',
    menuEmail: '我的邮箱地址',
    menuFaq: '常见问题',
    menuEsims: '我的 eSIM',
    menuOrders: '我的订单',
    menuGuide: '安装与激活说明',
    menuAbout: '关于我们',
    menuContact: '联系我们',
    menuLanguage: '语言设置',
    menuLogout: '退出登录',
    demo: '演示环境 · 数据已对接本地后端服务',
    logoutConfirm: '确定要退出登录吗？',
    loggedOut: '已退出登录',
    languageTitle: '切换语言'
  },
  contact: {
    heroTitle: '联系我们',
    heroSub: '我们随时为您服务',
    emailLabel: '官方邮箱',
    responseTip: '收到您的邮件后，我们将尽快回复，通常为 1-2 个工作日。',
    copyEmail: '复制邮箱',
    copied: '邮箱已复制'
  },
  about: {
    slogan: '全球流量，即买即用',
    version: '当前版本',
    introTitle: '关于 YYeSim',
    introText:
      'YYeSim 提供 200+ 国家与地区的 eSIM 流量套餐，扫码即装、落地即用，无需实体卡，让全球上网像本地流量一样简单。',
    tagRegions: '覆盖地区',
    tagSpeed: '网络速率',
    tagSupport: '在线支持',
    docAgreement: '用户协议',
    docAgreementSub: '服务内容、用户义务与免责条款',
    docPrivacy: '用户隐私政策',
    docPrivacySub: '我们如何收集、使用与保护你的信息',
    docRefund: '退款协议',
    docRefundSub: '未激活全额退款 · 已激活有权不退'
  },
  legal: {
    updated: '更新日期：{date}',
    date: '2026年9月',
    agreement: {
      title: '用户协议',
      date: '2026年9月',
      sections: [
        {
          h: '一、协议的接受',
          p: [
            '欢迎使用 YYeSim。本协议是您与 YYeSim 之间就使用本小程序服务所订立的契约。您在登录或使用本服务前，应仔细阅读并充分理解本协议全部内容。',
            '您一旦登录账号或以其他方式使用本服务，即表示您已阅读并同意接受本协议的全部条款。若您不同意本协议内容，请停止注册或使用本服务。'
          ]
        },
        {
          h: '二、服务内容',
          p: [
            '我们通过本小程序为您提供全球 200+ 国家与地区的 eSIM 流量套餐的展示、购买、激活码发放与管理服务。',
            'eSIM 套餐由合作的电信服务商提供，具体流量额度、有效期、网络制式与覆盖范围，均以套餐详情页展示的内容为准。'
          ]
        },
        {
          h: '三、账号与登录',
          p: [
            '您可通过支付宝授权方式登录并创建账号，账号用于关联您的订单与 eSIM 信息。',
            '您应妥善保管自己的账号及登录态，因您主动泄露、出借或保管不当造成的损失，由您自行承担。'
          ]
        },
        {
          h: '四、订单与支付',
          p: [
            '您在下单时应仔细核对套餐规格、购买方式、接收邮箱等信息，订单提交并支付成功后，eSIM 激活码将自动发放至您的账号。',
            '支付成功的订单如需退款，按《退款协议》的约定处理。'
          ]
        },
        {
          h: '五、用户义务',
          p: [
            '您承诺合法合规使用本服务，不得利用本服务从事任何违法违规活动，不得转售、倒卖套餐或激活码。',
            'eSIM 套餐仅供本人合理使用，需遵循公平使用原则，异常高频或恶意占用资源的行为，平台有权采取限速、停用等措施。'
          ]
        },
        {
          h: '六、知识产权',
          p: [
            '本小程序的商标、界面设计、程序代码及文档等知识产权，归 YYeSim 或相应权利人所有。',
            '未经授权，任何单位或个人不得复制、转载、反向工程或将其用于其他商业用途。'
          ]
        },
        {
          h: '七、免责声明',
          p: [
            '因当地运营商网络覆盖、信号质量、设备兼容性、目的地政策调整等非平台所能控制的原因，导致 eSIM 无法正常使用的，平台将尽力协助解决，但不承担由此产生的直接或间接损失。',
            '在法律允许的范围内，平台对服务中断、数据延迟等情形不承担违约责任。'
          ]
        },
        {
          h: '八、协议的变更与终止',
          p: [
            '我们可根据法律法规及运营需要对本协议进行修订，修订后的协议将在本页面更新公示。',
            '若您在协议修订后继续使用本服务，即视为接受修订后的内容；若您不同意，应停止使用本服务。'
          ]
        }
      ]
    },
    privacy: {
      title: '用户隐私政策',
      date: '2026年9月',
      sections: [
        {
          h: '一、引言',
          p: [
            '我们非常重视您的个人信息与隐私保护。本政策用于说明我们收集哪些信息、如何使用与保护这些信息。',
            '您使用本服务，即表示您已阅读并同意本政策所述的信息处理方式。'
          ]
        },
        {
          h: '二、我们收集的信息',
          p: [
            '账号信息：支付宝授权登录时获得的用户标识（OpenID）及昵称、头像；',
            '订单信息：购买的套餐、金额、支付状态，以及订单关联的 eSIM 卡号（ICCID）与激活码；',
            '联系信息：您在下单或个人中心主动填写的接收邮箱；',
            '设备信息：为保障账号与服务安全所必需的设备标识信息。'
          ]
        },
        {
          h: '三、信息的使用',
          p: [
            '创建并管理您的账号；',
            '处理订单、开通 eSIM 套餐并发放激活码；',
            '向您发送订单支付结果与 eSIM 激活提醒等通知；',
            '改进与优化我们的产品和服务。'
          ]
        },
        {
          h: '四、信息的存储与保护',
          p: [
            '我们采用加密传输、访问控制等安全技术和管理措施保护您的信息安全，防止信息被泄露、篡改或丢失。',
            '个人信息存储期限以实现处理目的所必需的最短时间为限，超期后将被删除或匿名化处理。'
          ]
        },
        {
          h: '五、信息的共享与披露',
          p: [
            '为完成发卡与支付，我们会在必要范围内向合作方提供您的相关信息：eSIM 服务合作方（用于套餐开通与管理）、支付宝（用于完成支付与退款）。',
            '未经您的同意，我们不会向其他第三方出售或提供您的个人信息，法律法规另有规定的除外。'
          ]
        },
        {
          h: '六、您的权利',
          p: [
            '您可以在小程序内查询、更正您的账号资料与接收邮箱。',
            '如需删除账号及相关数据，可通过客服渠道与我们联系，我们将在核实您的身份后依法处理。'
          ]
        },
        {
          h: '七、未成年人保护',
          p: [
            '本服务面向成年旅行用户。若您是未成年人，请在监护人的指导下使用本服务，并在监护人同意后提交个人信息。'
          ]
        },
        {
          h: '八、政策的更新',
          p: [
            '我们可能适时修订本政策，修订后将通过本页面公示。',
            '涉及重大变更时，我们会以适当方式提醒您注意。若您继续使用本服务，即表示同意修订后的政策。'
          ]
        }
      ]
    },
    refund: {
      title: '退款协议',
      date: '2026年9月',
      highlights: [
        {
          theme: 'ok',
          title: '未激活的 eSIM · 支持全额退款',
          desc: 'eSIM 卡在未安装、未激活的状态下申请退款，经审核通过后，将按实付金额全额退回。'
        },
        {
          theme: 'warn',
          title: '已激活的 eSIM · 有权不予退款',
          desc: 'eSIM 卡一旦安装或激活，套餐即开始消耗，平台有权拒绝该类订单的退款申请。'
        }
      ],
      sections: [
        {
          h: '一、生效时间与适用范围',
          p: [
            '本协议自 2026 年 9 月起生效。本协议修订后，以您提交退款申请当日正在执行的版本为准。',
            '本协议适用于您通过本小程序购买 eSIM 套餐所产生的全部订单（包括新购新卡与加购到已有 eSIM 的订单），您下单即视为已阅读并同意本协议。'
          ]
        },
        {
          h: '二、退款基本原则',
          p: [
            '退款以「eSIM 是否激活」为核心判断标准：未激活的订单支持申请退款；已激活的订单，平台有权不予退款。',
            'eSIM 一旦安装或激活，套餐流量与有效期即开始消耗，属于已消费的数字商品，原则上不支持无理由退款。'
          ]
        },
        {
          h: '三、支持退款的情况',
          p: [
            '1. eSIM 未安装、未激活（订单处于「待激活」状态）：无论何种原因（行程取消、买错套餐、重复购买等），均可申请全额退款；',
            '2. 因平台原因导致激活码无法发放或激活码无效：经核实后全额退款；',
            '3. 支付渠道异常导致重复扣款：平台将主动核实并退回多扣款项；',
            '4. 下单后 eSIM 尚未发放且您主动申请取消：可全额退款。'
          ]
        },
        {
          h: '四、有权不予退款的情况',
          p: [
            '1. eSIM 已安装或已激活，套餐已开始消耗；',
            '2. 套餐有效期已届满（已过期）；',
            '3. 因个人原因（设备不支持 eSIM、目的地变更等）在激活后申请退款；',
            '4. 下单前未确认手机是否支持 eSIM，且激活码已被核销使用（支持机型详见「安装与激活说明」）。'
          ]
        },
        {
          h: '五、特殊情况处理',
          p: [
            '1. 套餐无法使用：若 eSIM 在套餐支持的国家或地区内无法激活，或激活后完全无法使用，经平台与服务商核实确认后，可全额退款；',
            '2. 网络质量问题：若因当地运营商网络覆盖、信号等原因导致网速不达预期或间歇断网，属于第三方网络服务范畴，平台将协助排查并提供补偿方案（如更换套餐、顺延有效期），原则上不适用全额退款；',
            '3. 设备兼容问题：设备在「安装指南」列明的支持范围内却无法安装激活的，经客服核实后可退款；设备本身不支持 eSIM 的，按「是否激活」标准执行；',
            '4. 加购订单：为已到期 eSIM 加购的新套餐，以「新套餐是否激活」为标准：未激活可退，已激活不退；',
            '5. 不可抗力：因自然灾害、目的地政策调整等不可抗力导致服务无法提供的，双方友好协商解决。'
          ]
        },
        {
          h: '六、退款流程',
          p: [
            '第一步：进入「我的 → 我的订单」，找到对应订单，点击「申请退款」（仅待激活订单展示入口）；',
            '第二步：选择退款原因并填写补充说明，提交退款申请；',
            '第三步：平台在 1-3 个工作日内完成审核；',
            '第四步：审核结果将通过消息通知您；审核通过后退款自动发起，如被拒绝可查看拒绝理由；',
            '第五步：退款按原支付渠道退回您的支付宝账户，通常 1-7 个工作日内到账，具体以支付渠道处理进度为准。'
          ]
        },
        {
          h: '七、退款金额与到账方式',
          p: [
            '退款金额为订单实付金额，原路全额退回至您的支付宝账户，平台不收取任何手续费；',
            '使用优惠券、折扣等优惠方式下单的，按实际支付金额退回，优惠部分不折现、不补差。'
          ]
        },
        {
          h: '八、例外与限制',
          p: [
            '1. 存在恶意退款、批量下单退款、虚假申请等异常行为的，平台有权拒绝退款并限制账号功能；',
            '2. 违反《用户协议》被封禁的账号，其退款申请将转人工审核处理；',
            '3. 已删除的 eSIM 记录如需退款，请联系客服并提供订单号核实处理。'
          ]
        },
        {
          h: '九、协议更新',
          p: [
            '本协议可能根据法律法规及业务调整适时修订，修订后将更新本页面顶部展示的「更新日期」；',
            '涉及重大变更时，我们会以适当方式提醒您注意；若您在协议修订后继续使用本服务或提交退款申请，即视为接受修订后的内容；',
            '如本协议与《用户协议》存在冲突，以本协议关于退款的约定为准。'
          ]
        },
        {
          h: '十、联系我们',
          p: [
            '如对退款政策或退款进度有任何疑问，可通过小程序内客服渠道与我们联系，我们将尽快为您处理。'
          ]
        }
      ]
    }
  },
  emailPage: {
    title: '我的邮箱地址',
    sub: '接收订单与 eSIM 激活信息通知',
    cardTitle: '账号邮箱',
    notSet: '未设置邮箱',
    placeholder: '请输入邮箱地址',
    edit: '修改邮箱',
    setEmail: '设置邮箱',
    save: '保存',
    saving: '保存中...',
    invalid: '请填写正确的邮箱',
    saved: '邮箱已更新',
    tip: '邮箱用于接收订单支付结果与 eSIM 激活信息提醒，也可在下单时填写接收邮箱。',
    loginTitle: '登录后查看账号邮箱',
    loginSub: '登录后即可查看与修改接收通知的邮箱地址',
    goLogin: '去登录'
  },
  faq: {
    title: '常见问题',
    eyebrow: '帮助中心',
    sub: '订单、安装激活、上网与流量，一站式解答',
    guideEntry: '查看 eSIM 安装指南',
    guideEntrySub: 'iPhone / Android 图文安装步骤',
    countUnit: '个问题',
    searchPlaceholder: '搜索问题，如：退款 / 激活 / 流量',
    searchEmptyTitle: '没有找到相关问题',
    searchEmptySub: '试试输入「退款」「激活」「流量」等关键词',
    groups: [
      {
        icon: 'order',
        title: '订单与退款',
        items: [
          {
            q: '如何查看我的订单与状态？',
            a: '进入「我的 → 我的订单」页面可查看全部订单，每笔订单的状态会清晰标注为「待付款 / 待激活 / 已完成 / 已退款」。'
          },
          {
            q: '如何申请退款？',
            a: '仅「待激活」的订单可申请退款。进入订单详情点击「申请退款」，审核通过后款项将原路退回支付账户。'
          },
          {
            q: '退款多久到账？',
            a: '审核通过后，退款将按支付渠道原路退回，实际到账时间以支付平台处理为准。'
          },
          {
            q: '如何修改接收通知的邮箱？',
            a: '在「我的 → 我的邮箱地址」页面可随时查看或修改，用于接收订单与激活信息通知。'
          }
        ]
      },
      {
        icon: 'install',
        title: '安装与激活',
        items: [
          {
            q: '如何查看 eSIM 激活码？',
            a: '进入「我的 eSIM」页面，点击「查看激活码」，即可扫码安装或复制激活码手动安装。'
          },
          {
            q: '支持哪些设备？',
            a: 'iPhone XS/XR 及以上，以及部分三星、华为、小米等支持 eSIM 的机型，详见「安装与激活说明」中的设备兼容性检查。'
          },
          {
            q: '什么时候安装最合适？',
            a: '建议到达目的地后再购买并安装，避免提前占用有效期；套餐有效天数自购买之日起算。'
          },
          {
            q: '激活码有效期多久？',
            a: '激活码自购买后长期有效，套餐有效期自购买之日起算，建议您按出行计划购买、到达目的地后再安装。'
          }
        ]
      },
      {
        icon: 'signal',
        title: '上网与漫游',
        items: [
          {
            q: '到了国外没网怎么办？',
            a: '请确认 eSIM 已安装并开启「数据漫游」开关；若仍无法上网，可在系统设置中手动选择当地运营商网络。'
          },
          {
            q: '可以开个人热点共享吗？',
            a: '可以。大部分套餐支持开启个人热点，供同行的手机、平板等设备共享流量。'
          },
          {
            q: '上网速度如何？',
            a: '套餐均提供高速 4G/5G 网络，实际速度取决于当地运营商覆盖与信号环境。'
          }
        ]
      },
      {
        icon: 'data',
        title: '流量与套餐',
        items: [
          {
            q: '流量用完了怎么办？',
            a: '已到期的 eSIM 可在下单时选择「加购到已到期的 eSIM」，流量叠加、有效期顺延、激活码不变；也可以直接购买一张新 eSIM。'
          },
          {
            q: '如何查看剩余流量？',
            a: '在「我的 eSIM」页面可查看已购套餐的剩余流量与有效期。'
          },
          {
            q: '套餐有效期如何计算？',
            a: '套餐有效期自购买之日起算，具体天数可在套餐详情页查看；续费时流量累加、到期时间顺延。'
          }
        ]
      }
    ]
  },
  login: {
    subtitle: '全球流量，即买即用',
    btn: '支付宝一键登录',
    agreePrefix: '登录即表示同意',
    agreement: '《用户协议》',
    and: '和',
    privacy: '《隐私政策》',
    footer: '全球200+国家和地区可用',
    success: '登录成功',
    failed: '登录失败',
    authFailed: '授权失败，请重试',
    alipayOnly: '仅支持支付宝小程序登录',
    failedRetry: '登录失败，请重试'
  },
  countries: {
    searchPlaceholder: '搜索国家/地区，如 日本 / Japan',
    catAll: '全部',
    catAsia: '亚洲',
    catEurope: '欧洲',
    catAmericas: '美洲',
    catOceania: '大洋洲',
    catAfrica: '非洲',
    catMiddleEast: '中东',
    multiRegion: '多国通用套餐',
    nationalRegions: '国家与地区',
    empty: '没有找到「{kw}」，换个关键词试试',
    emptySearchTitle: '未找到「{kw}」',
    emptySearchSub: '试试其他关键词吧',
    emptyCatTitle: '该分类暂无国家',
    emptyCatSub: '试试其他分类吧',
    recentTitle: '最近搜索',
    clearAll: '清除',
    hotTitle: '热门目的地',
    plan: '流量套餐',
    sold: '已售 1000+',
    instant: '即时激活',
    noRealName: '扫码即装'
  },
  packages: {
    noRealName: '扫码即装',
    instant: '即买即用',
    daysSelectable: '{min}-{max}天可选',
    daysValid: '{days}天有效',
    multiData: '多种流量包',
    view: '查看',
    empty: '该目的地暂未上架套餐',
    tip: '购买后激活码将自动发放到「我的 eSIM」，扫码即可安装'
  },
  package: {
    // 覆盖类型
    coverageMulti: '{region}多国通用',
    coverageLocal: '{region}覆盖',
    // 套餐类型
    typeMulti: '多国通用',
    typeLocal: '本地套餐',
    // 标签
    tagHot: '热门',
    unlimited: '不限量',
    // 特性
    feature1: '即买即用，扫码秒激活',
    feature2: '全程高速 4G/5G 网络',
    feature3: '可开热点，多人共享',
    feature4: '无需换卡，即买即用',
    // 安装步骤
    step1: '购买后复制二维码下方的激活码',
    step2: '手机设置 → 蜂窝网络 → 添加 eSIM',
    step3: '扫码或输入激活码完成安装',
    step4: '到达目的地后开启数据漫游即用'
  },
  detail: {
    tabSelect: '套餐选择',
    tabDetail: '套餐详情',
    tabHot: '热门推荐',
    tabNotice: '使用须知',
    nameSuffix: '{name}流量套餐',
    sold: '已售 {n}',
    instant: '即时激活',
    noRealName: '扫码即装',
    globalApp: '支持全球app',
    warn: 'eSIM 需到达{name}当地后才能激活，且仅限在{name}境内使用，请抵达目的地后再激活！',
    selectDays: '选择天数',
    dayUnit: '{d}天',
    selectData: '选择数据用量包',
    perDay: '{unit}GB/天',
    totalGb: '总量 {gb}GB',
    unlimitedHint: '不限量套餐：含 {gb}GB 高速流量，用完后限速，仍可继续使用',
    dataPriceHint: '{days}天仅需{price}元',
    coverage: '覆盖地区：',
    detailTitle: '套餐详情',
    descTitle: '套餐说明',
    installTitle: '安装步骤',
    viewFullGuide: '查看完整安装指南 ›',
    noticeTitle: '温馨提示',
    noticeText:
      '1. 有效期从激活当日起算，请到达目的地后再激活。\n2. 流量遵循公平使用原则，超出高速额度后限速。\n3. 请先确认手机支持 eSIM 功能（iPhone XS 及以上等）。',
    dayUnitShort: '天',
    onlyNeed: '仅需',
    network: '通话和短信：',
    pkgTypeIntro: '套餐类型介绍',
    androidInstall: '安卓安装eSIM',
    appleInstall: '苹果安装eSIM',
    supportModels: '支持eSIM手机型号汇总',
    usageNotice: 'eSIM使用须知',
    selectPackage: '选择套餐',
    searchPlaceholder: '搜索国家/套餐',
    noResults: '暂无匹配结果',
    drawerTitle: '目的地选择',
    originalPrice: '原价 ¥{price}',
    discount: '立减{percent}%',
    buyNow: '立即购买',
    navTitle: 'eSIM 详情'
  },
  checkout: {
    eyebrow: 'CHECKOUT',
    instantChip: '即时激活',
    title: '商品信息',
    emailLabel: '接收邮箱',
    emailPlaceholder: '用于接收 eSIM 激活信息',
    emailTip: '购买后激活码将自动发放至「我的 eSIM」，邮箱仅用于消息提醒',
    payMethod: '支付方式',
    alipayName: '支付宝',
    alipayDesc: '安全快捷 · 支持余额/花呗/银行卡',
    amountLabel: '商品金额',
    discountLabel: '优惠',
    totalLabel: '应付总额',
    agree: '我已阅读并同意《购买服务协议》与《eSIM 使用须知》',
    payActual: '实付',
    submitting: '提交中...',
    submit: '提交订单',
    agreeFirst: '请先阅读并同意协议',
    emailInvalid: '请填写正确的邮箱',
    orderFailed: '下单失败',
    needLogin: '请先登录后再下单',
    needLoginTip: '登录后订单和 eSIM 将归入你的账号',
    skuName: '{name} eSIM',
    sumMeta: '{label} · {days}天 · {network}',
    buyMode: '购买方式',
    buyNew: '新购新卡',
    buyNewDesc: '购买一张新的 eSIM 卡并安装',
    buyAdd: '加购到已到期的 eSIM',
    buyAddDesc: '为已到期的 eSIM 卡充值新套餐',
    buyAddTo: '加购到',
    buyDayUnit: '天',
    buyExpire: '到期 {date}',
    buyClear: '清除',
    buySelectCard: '请选择要加购到的 eSIM 卡',
    buyAddEmpty: '暂无已到期的 eSIM 可加购',
    buyChoose: '选择卡片',
    buyDrawerTitle: '选择要加购的 eSIM',
    buyPreselectGone: '目标卡不可续费，已转为新购'
  },
  payment: {
    eyebrow: 'PAYMENT',
    amountLabel: '支付金额',
    orderNoPrefix: '订单号 {no}',
    goodsLabel: '商品',
    payMethodLabel: '支付方式',
    alipayBalance: '余额支付 · 尾号 8848',
    payNow: '立即支付',
    cancel: '暂不支付',
    noteReal: '将跳转至支付宝完成安全支付',
    noteDemo: '演示环境 · 点击立即支付将模拟支付宝扣款并自动发卡',
    success: '支付成功',
    successSub: 'eSIM 已自动发放至你的账户',
    orderNo: '订单号',
    pkgLabel: '套餐',
    actualAmount: '实付金额',
    viewEsims: '查看我的 eSIM',
    backHome: '返回首页',
    paying: '正在支付...',
    payingSub: '正在通过支付宝完成扣款',
    orderNotFound: '订单不存在',
    createFail: '创建支付失败',
    createFailRetry: '创建支付失败，请重试',
    payFail: '支付失败',
    payFailRetry: '支付失败，请重试',
    orderAbnormal: '订单信息异常',
    orderLabel: '{name} eSIM（{gb}GB / {days}天）'
  },
  orders: {
    eyebrow: 'MY ORDERS',
    detailEyebrow: 'ORDER DETAIL',
    heroTitle: '我的订单',
    heroSub: '共 {n} 笔订单',
    emptyTitle: '暂无订单',
    emptySub: '去挑选一张适合你的全球流量套餐吧',
    goBuy: '去购买',
    paid: '已支付',
    pending: '待支付',
    refunded: '已退款',
    tabAll: '全部',
    tabPending: '待付款',
    tabActivate: '待激活',
    tabDone: '已完成',
    tabRefunded: '已退款',
    noMatch: '该分类暂无订单',
    noMatchSub: '换个分类看看吧',
    meta: '{gb}GB · {days}天有效',
    orderNoLabel: '订单号',
    createdAtLabel: '下单时间',
    emailLabel: '接收邮箱',
    payMethodLabel: '支付方式',
    paidAtLabel: '支付时间',
    goPay: '去支付',
    detailNotFound: '订单不存在',
    retry: '重试',
    networkError: '网络异常，请稍后重试',
    back: '返回',
    esimInfo: 'eSIM 激活信息',
    iccid: 'ICCID',
    activationCode: '激活码',
    expireAt: '有效期至',
    orderInfo: '订单信息',
    typeLabel: '订单类型',
    typeNew: '新购',
    typeRenew: '续费',
    refundApply: '申请退款',
    refundApplying: '退款审核中',
    refundApplyingSub: '退款申请已提交，请耐心等待处理',
    refundRejected: '退款被拒绝',
    refundRejectedSub: '您的退款申请已被拒绝',
    refundRejectCountLeft: '已拒绝 {count} 次，还可申请 {remain} 次',
    refundRejectLimitReached: '退款申请被拒绝次数已达上限（{max} 次），无法再次申请',
    refundHistoryTitle: '退款记录',
    refundStepRequested: '申请',
    refundStepRejected: '已拒绝',
    refundStepApproved: '已同意',
    refundReasonLabel: '退款原因',
    refundReasonPlaceholder: '请填写退款原因（必填）',
    refundReasonRequired: '请填写退款原因后再提交',
    refundSubmit: '提交申请',
    refundAppliedToast: '退款申请已提交，请耐心等待',
    rejectReasonLabel: '拒绝理由',
    delete: '删除订单',
    deleteTitle: '删除订单',
    deleteConfirm: '确定要删除「{name}」这笔待付款订单吗？',
    deleteSuccess: '订单已删除',
    cancel: '取消'
  },
  esims: {
    title: '我的 eSIM',
    sub: '共 {n} 张 · 全球流量随时可用',
    buy: '+ 购买',
    emptyTitle: '还没有 eSIM',
    emptySub: '去挑选一张适合你的全球流量套餐吧',
    goBuy: '去购买',
    activated: '已激活',
    pending: '待激活',
    packageCount: '{n} 个套餐',
    currentPlan: '使用中套餐',
    activationDate: '激活时间',
    bindingId: '绑定 ID',
    tigerPackage: 'Tiger 套餐',
    used: '已用完',
    expired: '已过期',
    meta: '{gb}GB · {days}天有效',
    usage: '已用 {used}GB / {total}GB',
    expireLabel: '有效期至',
    qrTip: '扫描上方二维码，或复制激活码在手机「设置 → 蜂窝网络 → 添加 eSIM」中安装',
    copy: '复制',
    renew: '续费',
    markActivated: '标记已激活',
    collapse: '收起',
    viewCode: '查看激活码',
    delete: '删除',
    activateSuccess: '已标记为激活',
    copied: '激活码已复制',
    deleteTitle: '删除 eSIM',
    deleteConfirm: '删除后该 eSIM 将无法恢复，确定删除吗？',
    deleted: '已删除',
    deleteFailed: '删除失败'
  },
  esimDetail: {
    eyebrow: 'TRAVEL eSIM',
    specTitle: '套餐规格',
    specValue: '{gb}GB · {days}天',
    specGb: '{gb}GB',
    specDays: '{days}天',
    usageTitle: '流量使用情况',
    usedGb: '已用 {used}GB',
    totalGb: '共 {gb}GB',
    highSpeedGb: '高速 {gb}GB',
    unlimitedNote: '不限量套餐：高速流量用完后限速，仍可继续使用',
    qrTitle: 'eSIM 激活码',
    copied: '已复制',
    deleteConfirm: '确定要删除这张 eSIM 卡吗？删除后无法恢复。',
    loadFailed: '加载失败'
  },
  guide: {
    title: 'eSIM 安装指南',
    sub: '3 分钟搞定，全球上网不迷路',
    compatTitle: '设备兼容性检查',
    compatTxt:
      '支持 eSIM 的设备：iPhone XS/XR 及以上、iPad Pro 及以上、三星 Galaxy S20 及以上、华为 P40/Mate40 Pro 及以上（国行部分机型除外）、小米 12T Pro 及以上等。可前往手机「设置」中查看是否有「添加 eSIM」或「SIM 卡管理」入口。',
    faqTitle: '常见问题',
    stepsIos: [
      {
        image: '/static/guide/zh/ios-1.png',
        title: '连接 Wi-Fi 网络',
        desc: '前往「设置」页面，连接好 Wi-Fi 网络。'
      },
      {
        image: '/static/guide/zh/ios-2.png',
        title: '进入添加 eSIM',
        desc: '设置 → 蜂窝网络 → 点击「添加 eSIM」。'
      },
      {
        image: '/static/guide/zh/ios-3.png',
        title: '点击使用二维码',
        desc: '点击「使用二维码」。'
      },
      {
        image: '/static/guide/zh/ios-4.png',
        title: '点击扫描二维码',
        desc: '点击「扫描二维码」。'
      },
      {
        image: '/static/guide/zh/ios-5.png',
        title: '扫描 Tiger eSIM 二维码',
        desc: '扫描 Tiger eSIM 提供的二维码，然后稍等片刻。'
      },
      {
        image: '/static/guide/zh/ios-6.png',
        title: '开始激活',
        desc: '激活 eSIM，点击「继续」开始激活。'
      },
      {
        image: '/static/guide/zh/ios-7.png',
        title: '选择使用此 eSIM 的位置',
        desc: '选择使用此 eSIM 的位置，点击「继续」。'
      },
      {
        image: '/static/guide/zh/ios-8.png',
        title: '启用此号码并开启漫游',
        desc: '重要操作提醒：安装完成后，前往「设置 → 蜂窝网络」选择「启用此号码」，开启「数据漫游」功能，方可正常上网。'
      },
      {
        image: '/static/guide/zh/ios-9.jpeg',
        title: '无法扫码时手动输入',
        desc: '如果无法扫描二维码，可点击扫码取景框下方的「其他选项」，进入输入激活码页面，复制 Tiger eSIM「手动安装」下方的 SM-DP+ 地址和激活码。'
      },
      {
        image: '/static/guide/zh/ios-10.png',
        title: '粘贴到输入框完成激活',
        desc: '粘贴到输入框，确认码无需输入，点击「下一步」，重复以上步骤即可进行激活。'
      }
    ],
    stepsAndroid: [
      {
        image: '/static/guide/zh/and-1.jpeg',
        title: '连接 Wi-Fi 网络',
        desc: '前往「设置 → 连接 → 网络与互联网」，开启 Wi-Fi 并连接。'
      },
      {
        image: '/static/guide/zh/and-2.jpeg',
        title: '进入 SIM 管理器',
        desc: '设置 → 连接 → 点击「SIM 管理器」或「SIM 卡与移动网络」。'
      },
      {
        image: '/static/guide/zh/and-3.jpeg',
        title: '添加 eSIM',
        desc: '点击「添加 eSIM」或「添加移动套餐」。'
      },
      {
        image: '/static/guide/zh/and-4.jpeg',
        title: '扫码添加',
        desc: '选择「扫一扫」或「扫描运营商二维码」。'
      },
      {
        image: '/static/guide/zh/and-5.jpeg',
        title: '扫描 Tiger eSIM 二维码',
        desc: '将 Tiger eSIM 提供的二维码置于取景框内进行扫描。'
      },
      {
        image: '/static/guide/zh/and-6.jpeg',
        title: '确认添加',
        desc: '出现「要添加 eSIM 吗」提示时，点击「添加」按钮。'
      },
      {
        image: '/static/guide/zh/and-7.jpeg',
        title: '等待下载安装',
        desc: '需等待几分钟，进行下载安装。'
      },
      {
        image: '/static/guide/zh/and-8.jpeg',
        title: '启用 eSIM 并设为移动数据',
        desc: '下载完毕后，找到已安装的 eSIM 卡，点击「启用」或打开开关以激活 eSIM 套餐，移动数据选择 eSIM 卡。'
      },
      {
        image: '/static/guide/zh/and-9.jpeg',
        title: '开启数据漫游',
        desc: '重要操作提醒：安装完成后，请务必前往「设置 → 移动网络」选择安装的 eSIM 卡 → 开启「数据漫游」功能，方可正常上网。不同手机品牌路径略有差异。',
        tips: ['华为在「无线和网络」中', '小米需进入「双卡与移动网络」']
      },
      {
        image: '/static/guide/zh/and-10.png',
        title: '无法扫码时手动输入',
        desc: '如果无法扫描二维码，可点击扫码取景框下方的「输入激活码」，复制 Tiger eSIM 提供的手动安装下方的 SM-DP+ 地址和激活码。'
      },
      {
        image: '/static/guide/zh/and-11.jpeg',
        title: '粘贴到输入框完成激活',
        desc: '粘贴到输入框，点击「完成」，重复以上步骤即可进行激活。'
      }
    ],
    faqs: [
      {
        q: '激活码有效期多久？',
        a: '激活码自购买后长期有效，套餐有效期从「安装激活」当天起算，建议到达目的地后再安装。'
      },
      {
        q: '流量用完了怎么办？',
        a: '可在「我的 eSIM」页面购买新的套餐，或在首页选择相同地区再次下单，新激活码将叠加到已激活的 eSIM 上。'
      },
      {
        q: '可以开热点共享吗？',
        a: '可以。大部分套餐支持开启个人热点，供同行的手机、平板等设备共享流量。'
      },
      {
        q: '到了国外没网怎么办？',
        a: '请确认已安装 eSIM 并开启「数据漫游」开关；若仍无法上网，可尝试手动选择当地运营商网络。'
      }
    ]
  },
  supportedModels: {
    eyebrow: 'COMPATIBILITY',
    title: '支持 eSIM 机型汇总',
    sub: '涵盖主流品牌支持 eSIM 的手机型号，仅供参考，以手机系统设置中的「添加 eSIM」入口为准。'
  },
  usageNotice: {
    eyebrow: 'USAGE NOTICE',
    title: 'eSIM 使用须知',
    sub: '使用前请仔细阅读，保障您的出行顺利',
    sections: [
      {
        title: '基本说明',
        items: [
          'eSIM 是一种内置于手机的嵌入式 SIM 卡，无需物理插卡，扫码或输入激活码即可开通全球流量套餐。',
          '本平台提供的 eSIM 套餐由合作的电信服务商提供，流量额度、有效期、网络制式与覆盖范围以套餐详情页展示为准。'
        ]
      },
      {
        title: '激活说明',
        items: [
          '请在到达目的地后再安装激活，有效期自激活当日起算，过早激活会提前消耗有效期。',
          '安装时请保持网络连接（可使用 Wi-Fi），按照安装指南操作即可完成激活。',
          '同一台设备可同时安装多张 eSIM，但同一时间仅可启用一张用于蜂窝网络。'
        ]
      },
      {
        title: '数据漫游',
        items: [
          '激活后请务必在手机设置中开启「数据漫游」开关，否则无法正常上网。',
          '到达目的地后若无法上网，可尝试在系统设置中手动选择当地运营商网络。'
        ]
      },
      {
        title: '公平使用原则',
        items: [
          '套餐遵循公平使用原则（Fair Usage），请用于正常个人用途。',
          '异常高频或恶意占用资源的行为，平台有权采取限速、停用等措施。',
          '不限量套餐在超出高速额度后会自动限速但不会断网，仍可继续使用。'
        ]
      },
      {
        title: '有效期与充值',
        items: [
          '套餐有效期为自激活当日起算的固定天数，到期后流量即失效。',
          '可在「我的 eSIM」查看已购套餐的剩余流量与有效期。',
          '套餐到期后，可下单「加购到已到期的 eSIM」，流量叠加、有效期顺延、激活码不变。'
        ]
      },
      {
        title: '设备兼容',
        items: [
          '请确认手机支持 eSIM 功能，可前往手机「设置」查看是否有「添加 eSIM」或「SIM 卡管理」入口。',
          '支持机型汇总详见「支持 eSIM 机型」页面；国行部分机型可能不支持 eSIM。'
        ]
      },
      {
        title: '退款说明',
        items: [
          '未安装、未激活的 eSIM 支持申请全额退款；已激活的 eSIM 有权不予退款。',
          '详细规则请阅读《退款协议》与《购买服务协议》。'
        ]
      },
      {
        title: '联系我们',
        items: [
          '如遇任何问题，可通过「我的 → 联系客服」或邮件联系我们，我们将尽快为您处理。'
        ]
      }
    ]
  }
}
