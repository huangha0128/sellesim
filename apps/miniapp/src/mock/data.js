...
export const REGIONS = [
  { code: 'GLOBAL', name: '全球通用', en: 'Global', flag: '/static/icons/region-global.png', tier: 4, cat: '全球', intro: '200+ 国家与地区通用' },
  { code: 'ASIA', name: '亚洲多国', en: 'Asia', flag: '/static/icons/region-asia.png', tier: 3, cat: '全球', intro: '覆盖 30+ 亚洲国家和地区' },
  { code: 'EUROPE', name: '欧洲多国', en: 'Europe', flag: '/static/icons/region-europe.png', tier: 3, cat: '全球', intro: '覆盖 40+ 欧洲国家和地区' },
  { code: 'AMERICAS', name: '美洲多国', en: 'Americas', flag: '/static/icons/region-americas.png', tier: 3, cat: '全球', intro: '覆盖 30+ 美洲国家和地区' },
  { code: 'OCEANIA', name: '大洋洲多国', en: 'Oceania', flag: '/static/icons/region-oceania.png', tier: 3, cat: '全球', intro: '澳新及太平洋群岛' }
]
...
export function getPackages(countryCode) {
  ...
  return BASE_PLANS.map((p, i) => {
    const raw = region ? REGION_PLAN_PRICES[region.code][i] : p.base * TIER_MULT[c.tier]
    return {
      id: `${countryCode}-${p.gb}G${p.days}D`,
      countryCode: c.code,
      countryName: c.name,
      flag: c.flag,
      gb: p.gb,
      ...
      tagColor: p.tagColor,
      desc: region
        ? `${region.name}通用数据套餐，跨区漫游免切换，${p.gb}GB 流量 ${p.days} 天有效。`
...
export function getHotPackages(n = 6) {
  ...
  return hot.map((c) => getPackages(c.code).find((p) => p.tag === '热销') || getPackages(c.code)[1])
}
