...
export const api = {
  ...
  async getPackages(countryCode) {
    const res = await request('GET', `/packages?countryCode=${countryCode}`);
  ...
  async createOrder({ pkgId, email, payMethod = 'alipay' }) {
    return request('POST', '/orders', { pkgId, email, payMethod });
  },
