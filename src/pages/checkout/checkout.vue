    ...
    <view v-if="pkg" class="checkout-body">
      <view class="section-card">
        ...
        <view class="pkg-summary">
          <view class="sum-flag">
            <text class="sum-flag-emoji">{{ pkg.flag }}</text>
          </view>
          <view class="sum-main">
      ...
      <view class="section-card">
        ...
        <view class="form-tip">💡 购买后激活码将自动发放至「我的 eSIM」，邮箱仅用于消息提醒</view>
      </view>
      ...
      <view class="agree-row" @click="agreed = !agreed">
        <view class="agree-box" :class="{ checked: agreed }">✓</view>
...
export default {
  ...
  computed: {
    priceNum() {
      if (!this.pkg || !this.pkg.dataPackages) return 0
      const dataPkg = this.pkg.dataPackages[this.dataIndex]
      if (!dataPkg) return 0
      const price = calcPrice(dataPkg, this.days, this.pkg.countryCode)
      return price % 1 === 0 ? price.toFixed(0) : price.toFixed(1)
    }
  },
  onLoad(options) {
    this.pkgId = options.pkgId || ''
    this.dataIndex = Number(options.dataIndex) || 0
    this.days = Number(options.days) || 7
    this.load()
  },
  methods: {
    async load() {
      ...
      try {
        ...
        // 设置数据用量包标签
        if (this.pkg && this.pkg.dataPackages && this.pkg.dataPackages[this.dataIndex]) {
          this.dataLabel = this.pkg.dataPackages[this.dataIndex].label
        }
      } finally {
        uni.hideLoading()
      }
